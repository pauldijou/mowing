(function() {
  ...
  Map = (function() {
    ...
    Map.prototype.addMower = function(mower) {
      if (this.isOutside(mower.x, mower.y)) {
        return notify("Dude, you can't put a mower outside of the field... " + (mower.toString()) + " will be ignored.", "error");
      } else if (this.hasMowerAt(mower.x, mower.y)) {
        return notify("Hey man, are you trying to fusion two mowers? " + (mower.toString()) + " will be ignored.", "error");
      } else {
        mower.setMap(this);
        mower.draw();
    ...
    Map.prototype.draw = function() {
      ...
      for (yy = _i = 0, _ref = this.y; 0 <= _ref ? _i <= _ref : _i >= _ref; yy = 0 <= _ref ? ++_i : --_i) {
        bodyMap.append('<tr></tr>');
        for (xx = _j = 0, _ref1 = this.x; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; xx = 0 <= _ref1 ? ++_j : --_j) {
          bodyMap.children('tr:last-child').append("<td id=\"square-" + xx + "-" + (this.y - yy) + "\" title=\"(" + xx + " x " + (this.y - yy) + ")\" rel=\"tooltip\"></td>");
        }
      }
      $('[rel="tooltip"]').tooltip();
      return divMap;
  ...
  Mower = (function() {
    ...
    Mower.prototype.advance = function() {
      ...
      if (this.map.isOutside(newX, newY)) {
        return notify("I don't know who gives orders but " + (this.toString()) + " just tried to go outside at (" + newX + "," + newY + ")", 'error');
      } else if (this.map.hasMowerAt(newX, newY)) {
        return notify("Saperlipopette!! We just avoid a crash between " + (this.toString()) + " and another mower at (" + newX + "," + newY + ")", 'error');
      } else {
        this.clean();
        this.x = newX;
        this.y = newY;
    ...
    Mower.prototype.toString = function() {
      return "mower #" + this.id + " at (" + this.x + ", " + this.y + ", " + this.d + ")";
    };
